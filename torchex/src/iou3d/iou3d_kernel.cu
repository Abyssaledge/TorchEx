...
__global__ void boxes_overlap_mixed_kernel(const int num_a, const float *boxes_a, const int *batch_id_a,
  ...
  if (batch_id_a[a_idx] != batch_id_b[b_idx]){
    return;
  }
  const float *cur_box_a = boxes_a + a_idx * 5;
  const float *cur_box_b = boxes_b + b_idx * 5;
...
__global__ void boxes_iou_bev_1to1_kernel(const int num, const float *boxes_a,
...
__global__ void nms_mixed_kernel(const int boxes_num, const float nms_overlap_thresh, const float *boxes,
                           const int *batch_id, unsigned long long *mask, bool normal) {
  // params: boxes (N, 5) [x1, y1, x2, y2, ry]
  // params: mask (N, N/THREADS_PER_BLOCK_NMS)
  ...
  if (threadIdx.x < row_size) {
    const int cur_box_idx = THREADS_PER_BLOCK_NMS * row_start + threadIdx.x;
    const int cur_box_batch_id = batch_id[cur_box_idx];
    const float *cur_box = boxes + cur_box_idx * 5;
    const int col_blocks = DIVUP(boxes_num, THREADS_PER_BLOCK_NMS);
    if (row_start > col_start) {
      mask[cur_box_idx * col_blocks + col_start] = 0ULL;
      return;
    }
    ...
    for (i = start; i < col_size; i++) {
      bool flag = (cur_box_batch_id == batch_id[THREADS_PER_BLOCK_NMS * col_start + i]);
      if (flag){
        if (normal)
          flag = iou_normal(cur_box, block_boxes + i * 5) > nms_overlap_thresh;
        else
          flag = iou_bev(cur_box, block_boxes + i * 5) > nms_overlap_thresh;
      }
      if (flag) {
        t |= 1ULL << i;
      }
...
void boxesoverlapMixedLauncher(const int num_a, const float *boxes_a, const int *batch_id_a,
  ...
  boxes_overlap_mixed_kernel<<<blocks, threads>>>(num_a, boxes_a, batch_id_a, num_b, boxes_b,
                                            batch_id_b, ans_overlap);
#ifdef DEBUG
  cudaDeviceSynchronize();  // for using printf in kernel function
#endif
}
...
void nms_mixed_Launcher(const float *boxes, unsigned long long *mask, int boxes_num,
  ...
  dim3 blocks(DIVUP(boxes_num, THREADS_PER_BLOCK_NMS),
              DIVUP(boxes_num, THREADS_PER_BLOCK_NMS));
  dim3 threads(THREADS_PER_BLOCK_NMS);
  nms_mixed_kernel<<<blocks, threads>>>(boxes_num, nms_overlap_thresh, boxes, batch_id, mask, normal);
}