...
void boxesoverlapLauncher(const int num_a, const float *boxes_a,
                          ...
                          float *ans_overlap);
void boxesoverlapMixedLauncher(const int num_a, const float *boxes_a, const int *batch_id_a,
                               const int num_b, const float *boxes_b, const int *batch_id_b,
                               float *ans_overlap);
...
void nms_mixed_Launcher(const float *boxes, unsigned long long *mask, int boxes_num,
                 const int *batch_id, float nms_overlap_thresh, bool normal);
...
int boxes_overlap_bev_mixed_gpu(at::Tensor boxes_a, at::Tensor boxes_b,
                                at::Tensor batch_id_a, at::Tensor batch_id_b,
                                at::Tensor ans_overlap) {
  // params boxes_a: (N, 5) [x1, y1, x2, y2, ry]
  // params boxes_b: (M, 5)
  // params batch_id_a: (M, ) The batch index of each box
  // params batch_id_b: (M, )
  // params ans_overlap: (N, M)

  CHECK_INPUT(boxes_a);
  CHECK_INPUT(boxes_b);
  CHECK_INPUT(batch_id_a);
  CHECK_INPUT(batch_id_b);
  CHECK_INPUT(ans_overlap);
  ...
  const float *boxes_a_data = boxes_a.data_ptr<float>();
  const float *boxes_b_data = boxes_b.data_ptr<float>();
  const int *batch_id_a_data = batch_id_a.data_ptr<int>();
  const int *batch_id_b_data = batch_id_b.data_ptr<int>();
  float *ans_overlap_data = ans_overlap.data_ptr<float>();

  boxesoverlapMixedLauncher(num_a, boxes_a_data, batch_id_a_data,
                            num_b, boxes_b_data, batch_id_b_data,
                            ans_overlap_data);
  return 1;
}
...
int boxes_iou_bev_1to1_gpu(at::Tensor boxes_a, at::Tensor boxes_b,
                      at::Tensor ans_iou) {
  // params boxes_a: (N, 5) [x1, y1, x2, y2, ry]
  // params boxes_b: (N, 5)
...
int nms_mixed_gpu(at::Tensor boxes, at::Tensor keep, at::Tensor batch_id,
	    float nms_overlap_thresh, bool normal) {
  // params boxes: (N, 5) [x1, y1, x2, y2, ry]
  // params keep: (N)
  // params batch_id: (N)
  ...
  int boxes_num = boxes.size(0);
  const float *boxes_data = boxes.data_ptr<float>();
  const int * batch_id_data = batch_id.data_ptr<int>();
  long *keep_data = keep.data_ptr<long>();
  ...
  CHECK_ERROR(cudaMalloc((void **)&mask_data,
                         boxes_num * col_blocks * sizeof(unsigned long long)));
  nms_mixed_Launcher(boxes_data, mask_data, boxes_num, batch_id_data, nms_overlap_thresh, normal);
...
int aug_nms_gpu(at::Tensor boxes, at::Tensor keep, at::Tensor labels,
      float nms_overlap_thresh, bool normal) {
  // params boxes: (N, 5) [x1, y1, x2, y2, ry]
  // params keep: (N)
  // params labels: (N)
  ...
  CHECK_ERROR(cudaMalloc((void **)&mask_data,
                         boxes_num * col_blocks * sizeof(unsigned long long)));
  nms_mixed_Launcher(boxes_data, mask_data, boxes_num, labels_data, nms_overlap_thresh, normal);
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("boxes_overlap_bev_mixed_gpu", &boxes_overlap_bev_mixed_gpu,
  ...
  m.def("aug_nms_gpu", &aug_nms_gpu, "augmented nms (CUDA)");
}
