...
// A simple hash table
__device__ __forceinline__ int double_hash(const long long key, int probe_i, int table_size);
__device__ __forceinline__ int double_hash(const long long key, int probe_i, int table_size){
...
__device__ void setvalue(const long long key, const int value, int *table, const int table_size);
__device__ void setvalue(const long long key, const int value, int *table, const int table_size){
  for (int i = 0; i < table_size; i++){
    int slot_idx = double_hash(key, i, table_size);
    // if (slot_idx < 0 || slot_idx >= table_size){
    //   printf("slot_idx: %d, table_size: %d, probe: %d, key: %d\n", slot_idx, table_size, i, key);
    //   assert(false);
    // }
...
__device__ int getvalue(const long long key, const int *table, const int table_size);
__device__ int getvalue(const long long key, const int *table, const int table_size){
  for (int i = 0; i < table_size; i++){
    int slot_idx = double_hash(key, i, table_size);
