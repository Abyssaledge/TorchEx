...
__global__ void generate_pts_mask_for_box3d_mixed(
  const int boxes_num, const int pts_num, const int max_pts_num, const int max_all_pts_num,
  const float *rois,
  ...
  // params rois: (N, 7) [x, y, z, w, l, h, rz] in LiDAR coordinate
  // params rois_batch: (N, ) the batch index of each roi
  // params pts: (npoints, 3) [x, y, z]
  // params pts_batch: (npoints, ) the batch index of each point
  // counter: [num_box, ]
  // pts_mask [num_box, max_pts_num] 
  // pts_mask_info [num_box, max_pts_num, info_dim] 
  ...
  float local_x = 0, local_y = 0;
  int cur_in_flag = 0;
  if (pts_batch[pt_idx] == rois_batch[box_idx]){
    cur_in_flag = check_pt_in_box3d(pts, rois, local_x, local_y, extra_w, extra_l, extra_h);
  }
...
void dynamic_point_pool_mixed_launcher(
  ...
  generate_pts_mask_for_box3d_mixed<<<blocks_mask, threads>>>(
    boxes_num, pts_num, max_num_pts_per_box, max_all_pts_num,
    rois,
    rois_batch,
    pts,
